...
pub(super) fn compute<T: FactTypes>(
    ...
    let (errors, subset_errors) = {
        ...
        let res = infer_run!{
            ...
            origin_live_on_entry(*o, *p) <-- for (o, p) in origin_live_on_entry_rel.elements.iter();
            lattice all_origin_live_on_entry(infer::lattice::set::Set<(T::Origin, T::Point)>);
            ...
            live_to_dying_regions(*origin1, *origin2, *point1, *point2) <--
              ...
              cfg_edge(point1, point2),
              origin_live_on_entry(origin1, point2),
              all_origin_live_on_entry(origin_live_on_entry_all) if !origin_live_on_entry_all.contains(&(origin2.clone(), point2.clone()));
            //   !origin_live_on_entry(origin2, point2);
        ...
        println!("res.loan_invalidated_at.len(): {}, res.placeholder_origin.len(): {}, res.cfg_edge.len(): {}
                  , res.subset.len(): {}", 
                    res.loan_invalidated_at.len(), 
                    res.placeholder_origin.len(),
                    res.cfg_edge.len(),
                    res.subset.len());
